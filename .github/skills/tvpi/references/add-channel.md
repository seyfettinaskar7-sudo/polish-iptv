# Add a Channel (seyfettinaska7-sudo/polish-iptv)

You add a live channel to **seyfettinaskar7-sudo/polish-iptv**, which serves Polish live TV as M3U playlists through two paths fed off the same channel list: a Cloudflare **Worker** (`worker/src/index.ts`, request-time resolution, recommended) and a **raw GitHub mirror** (`generate.py` → `streams/*.m3u`, committed every 15 min). A channel must be added to **both** files or the two paths disagree — the Worker would 404 a slug the mirror serves, or vice-versa.

The single most important habit: **keep `CHANNELS` (TVPVOD.ts) and `TVP_CHANNELS`/static lists (generate_tvp.py) in lockstep** — same `slug`, `name`, `logo`, `group`, and `id`. Read both files before editing.

## How the project fits together

```
.
├── worker/SRC/TVPVOD.ts        # Worker: CHANNELS[] + per-request L1→L2→L3a→L3b resolution; auto-deploys on push to worker/**
├── generate_tvp.py                # TVP_CHANNELS[]; pure-stdlib; writes streams/*.m3u (cron via refresh.yml)
...
### 3. Add to the Worker (`worker/SRC/TVPVOD.ts`)
...
### 4. Add to the generator (`generate_tvp.py`)
...
# 2) Worker still type-checks (you edited TVPVOD.ts):
